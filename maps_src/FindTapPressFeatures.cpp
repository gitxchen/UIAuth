...
int main(int argc,char *argv[]) {
  ...
  while(getline(fin,line)) {
    if(line.find("Down")!=string::npos) {
      ...
      s4="";
      found_down=1;
      string down_str(line);
      vector<string> v_s_down;
      v_s_down=split(down_str,' ',v_s_down);
      //cout<<down_str<<endl;//" "<<v_s_down[v_s_down.size()-1].c_str()<<endl;
      e.down_t=string_to_int(v_s_down[v_s_down.size()-1].c_str());
      getline(fin,s1);
      getline(fin,s2);
      //cout<<s1<<endl;
      //cout<<s2<<endl;
    ...
    if(line.find("Long Press")!=string::npos) {
      if(found_down!=1) continue;
      found_down=0;
      string up_str(line);
      //cout<<up_str<<endl;
      vector<string> v_s_up;
      v_s_up=split(up_str,' ',v_s_up);
      e.up_t = string_to_int(v_s_up[v_s_up.size()-1]);
