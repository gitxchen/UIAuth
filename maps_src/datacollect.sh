#!/bin/bash
cd $HOME/maps_logs/;
c++ $HOME/maps_src/FindTapFeatures.cpp -o $HOME/maps_logs/FindTapFeatures;
c++ $HOME/maps_src/FindBLTapFeatures.cpp -o $HOME/maps_logs/FindBLTapFeatures;
c++ $HOME/maps_src/FindTapFlingFeatures.cpp -o $HOME/maps_logs/FindTapFlingFeatures;
c++ $HOME/maps_src/FindTapPressFeatures.cpp -o $HOME/maps_logs/FindTapPressFeatures;
c++ $HOME/maps_src/FindTapScrollFeatures.cpp -o $HOME/maps_logs/FindTapScrollFeatures;
...
if [ "$1" == "--button" ]; then
  SEARCH_STRINGS="GO_BUTTON";
  LOG_PREFIX="button";
  BINARY="FindTapFeatures"
fi

if [ "$1" == "--label" ]; then
  SEARCH_STRINGS="DIRECTIONSLABEL";
  LOG_PREFIX="label";
  BINARY="FindTapFeatures"
fi

if [ "$1" == "--buildinglist" ]; then
#Need to mod 67 every y coordinate
  SEARCH_STRINGS="BUILDINGLIST";
  LOG_PREFIX="bl";
  BINARY="FindBLTapFeatures"
fi

if [ "$1" == "--routinglistfragment" ]; then
  SEARCH_STRINGS="ROUTINGLISTFRAGMENT";
  LOG_PREFIX="rlf";
  BINARY="FindTapFeatures"
fi
...
do
  ...
  #rm $HOME/maps_logs/maps_logs/$user-$LOG_PREFIX-downup.log;
  #rm $HOME/maps_logs/maps_logs/$user-$LOG_PREFIX-test-downup.log;
  cd $HOME/maps_logs/$user;
  rm *-$LOG_PREFIX*;
  for search_string in $SEARCH_STRINGS
  do
    for b in $BINARY
    do
      cp $HOME/maps_logs/$b $HOME/maps_logs/$user;
      $HOME/maps_logs/$user/$b $search_string maps.txt $user-$LOG_PREFIX-maps;
      rm $HOME/maps_logs/$user/$b;
    done
  done
  mv $HOME/maps_logs/$user/$user-$LOG_PREFIX-maps-downup.log $HOME/maps_logs/maps_logs;
  #mv $HOME/maps_logs/$user/$user-$LOG_PREFIX-test-downup.log $HOME/maps_logs/maps_logs;
  cd $HOME/maps_logs;
done
